...
function timeAllWeek(state) {
    ...
    if (tokens.length === 1 && tokens[0].type === 'time' && tokens[0].value.allDay) {
        tokens.splice(0, 0, {
            type: 'days',
    ...
    // if just two tokens with time add a through token
...
function throughOp(state, index) {
    ...
    if (prev.type !== next.type || (prev.type !== 'days' && prev.type !== 'time')) {
        ...
        prev.value = [];
        var totalDaysToAdd = endDayIdx >= startDayIdx ?
            endDayIdx - startDayIdx + 1 :
            (utils.daysOfWeek.length - startDayIdx) + endDayIdx + 1;

        var current = startDayIdx;
        while (totalDaysToAdd) {
            i = current++ % utils.daysOfWeek.length;
...
function getTimeProfile(timeRanges) {
    ...
    var timeProfile = {};
    var i, timeRange, end, time, timeStr;

    for (i = 0; i < timeRanges.length; i++) {
        timeRange = timeRanges[i];
        time = normalizeTime(timeRange.start);
        end = normalizeTime(timeRange.end);
        ...
        while (time !== end) {
            ...
            timeStr = getTimeStringFromTime(time);
...
function normalizeTime(time) {
    ...
    var hour = Math.floor(time / 100); // eg 830 => 8
    var minutes = (time % 100 >= 30) ? 30 : 0;
 ...
 * @param state
 */
function getDayTimes(state, options) {
    var dayTimes = {};
    var tokens = state.tokens || [];
    ...
    while (i < upperBoundry) {
        ...
        for (j = 0; j < dayToken.value.length; j++) {
            ...
            if (timeToken.value.closed) {
                isAllDayEveryDay = false;
                dayTimes[day].closed = true;
            }
            else if (timeToken.value.allDay) {
                ...
                if (options && options.rangesOnly) {
                    dayTimes[day] = timeToken.value.ranges;
                }
                else {
...
function parse(hoursText, options) {
    ...
    return getDayTimes(state, options);                     // get object with combined days and times
}
