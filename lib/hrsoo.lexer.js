...
function checkCommonHours(state) {
    ...
    text = text.replace(/24\s?[-/x]\s?7/g, '24 hours, 7 days');
    text = text.replace(/^every ?day$/g, '24 hours, 7 days');
    text = text.replace(/^always$/g, '24 hours, 7 days');
    text = text.replace(/every ?day/g, '7 days');
    text = text.replace(/noon/g, '12pm');
    text = text.replace(/â€“/g, 'through');
    text = text.replace(/ã¢â‚¬â€œ/g, 'through');
    text = text.replace(/\//g, ' ');
    text = text.replace(/daily/g, '7 days');
    text = text.replace('7 days a week', '7 days');
    text = text.replace('seven days a week', '7 days');

    if (/24 ho?u?rs/.test(text)) {
        state.tokens.push({
            type: 'time',
            value: {
                allDay: true
    ...
    if (/^(closed|never|none)$/.test(text)) {
        state.tokens.push({
            type: 'days',
